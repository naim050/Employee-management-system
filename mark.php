...
if (isset($_POST['update'])) {
    ...
    $stmt1 = mysqli_prepare($conn, "UPDATE `project` SET `mark` = ? WHERE eid = ? AND pid = ?");
    mysqli_stmt_bind_param($stmt1, "iii", $mark, $eid, $pid);
    mysqli_stmt_execute($stmt1);

    $stmt2 = mysqli_prepare($conn, "UPDATE `rank` SET `points` = ? WHERE eid = ?");
    mysqli_stmt_bind_param($stmt2, "ii", $upPoint, $eid);
    mysqli_stmt_execute($stmt2);

    $stmt3 = mysqli_prepare($conn, "UPDATE `salary` SET `bonus` = ?, `total` = ? WHERE id = ?");
    mysqli_stmt_bind_param($stmt3, "iii", $upBonus, $upSalary, $eid);
...
$sql = "SELECT pid, project.eid, pname, duedate, subdate, mark, points, firstName, lastName, base, bonus, total FROM `project`, `rank`, `employee`, `salary`  WHERE project.eid = ? AND pid = ? AND project.eid = rank.eid AND salary.id = project.eid AND employee.id = project.eid AND employee.id = rank.eid";
...
<html>
<head>
  <title>Project Mark | FUTURE TEN</title>
  <!-- Icons font CSS-->
